...
export function App() {
  ...
  // Condição atualizada para ver o menu Administração
  ...
  const allNavItems = [
    ...
    { id: 'grupos', label: 'Grupos de Cuidado', icon: Users, show: perms?.grupos?.view },
    { id: 'escalas', label: 'Escalas', icon: CalendarCheck, show: perms?.escalas?.view },
    { id: 'share', label: 'QR Code Ficha', icon: QrCode, show: perms?.qrcode?.view },
    { id: 'settings', label: 'Administração', icon: Settings, show: canViewSettings },
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex font-sans">
      ...
      <aside className="fixed left-0 top-0 h-screen bg-white border-r border-stone-200 w-[72px] hover:w-64 transition-all duration-300 z-50 flex flex-col overflow-hidden group shadow-[4px_0_24px_rgba(0,0,0,0.02)] print:hidden">
        ...
        <div className="p-4 shrink-0 border-t border-stone-100">
          <button onClick={() => supabase.auth.signOut()} className="flex items-center px-3 py-3 w-full text-left text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors font-semibold">
      ...
      <div className="flex-1 ml-[72px] flex flex-col min-h-screen">
        <header className="bg-white/70 backdrop-blur-xl border-b border-stone-200 px-8 min-h-[72px] py-3 flex justify-between items-center sticky top-0 z-40 print:hidden">
          <div>
            {currentTab === 'inicio' ? (
              <>
                <h1 className="text-xl font-black text-stone-800 tracking-tight">Olá, {firstName}</h1>
                <p className="text-stone-500 text-xs sm:text-sm font-medium mt-0.5">Bem-vindo(a) ao painel da sua congregação. Fique por dentro dos próximos passos da nossa comunidade.</p>
              </>
            ) : (
              <h1 className="text-xl font-bold text-stone-800 capitalize tracking-tight">
                {navItems.find(i => i.id === currentTab)?.label || 'Painel'}
              </h1>
            )}
          </div>
          <div className="flex items-center gap-3 shrink-0 ml-4">
            <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-stone-200 shadow-sm text-stone-700 text-sm font-bold">
              <span>{userProfile?.full_name}</span>
            </div>
          </div>
        </header>

        <main className="p-6 sm:p-10 max-w-6xl mx-auto w-full flex-1">
  ...
  return (
    <div className="flex min-h-screen font-sans selection:bg-amber-100 selection:text-amber-900 relative">
      ...
      <div className="flex-1 flex items-center justify-center p-4 relative z-10">
        <div className="w-full max-w-[400px] bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl shadow-stone-300/50 p-8 sm:p-12 border border-white">
          ...
          <h2 className="text-3xl font-black text-stone-800 mb-2 tracking-tight text-center">Bem-vindo</h2>
          <p className="text-base text-stone-500 mb-8 font-medium text-center">Acesse sua conta para continuar.</p>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">E-mail</label>
              <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(null); }} required className="w-full px-4 py-3.5 bg-white border border-stone-200 rounded-xl text-base focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none text-stone-800 transition-all shadow-sm" />
            </div>
            <div>
              <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Senha</label>
              <input type="password" value={password} onChange={e => { setPassword(e.target.value); setError(null); }} required className="w-full px-4 py-3.5 bg-white border border-stone-200 rounded-xl text-base focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none text-stone-800 transition-all shadow-sm" />
            </div>
            <button type="submit" disabled={loading} className="w-full py-4 bg-amber-600 hover:bg-amber-700 text-white font-bold text-lg rounded-xl transition-all shadow-lg shadow-amber-600/20 mt-4 active:scale-[0.98] disabled:opacity-70">
              {loading ? 'Autenticando...' : 'Entrar na Plataforma'}
            </button>
          </form>

          <div className="mt-10 pt-8 border-t border-stone-100 text-center">
            <p className="text-sm text-stone-500 font-medium">Não tem uma conta? <button onClick={() => setIsRegistering(true)} className="text-amber-700 font-bold hover:underline ml-1">Registar agora</button></p>
          </div>
        </div>
      </div>
    </div>
